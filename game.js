class TreasureHuntGame {
    ...
    createConfettiExplosion(markerId) {
        console.log('🎊 Explosion de confetti!');
        
        const arContainer = document.getElementById(`ar-container-${markerId}`);
        if (!arContainer) return;
        
        // Créer 30 confetti qui explosent
        for (let i = 0; i < 30; i++) {
            const confettiCanvas = document.createElement('canvas');
            confettiCanvas.width = 64;
            confettiCanvas.height = 64;
            ...
            confetti.setAttribute('position', `0 0.5 0`);
            confetti.setAttribute('width', '0.1');
            confetti.setAttribute('height', '0.1');
            confetti.setAttribute('material', 'transparent: true; side: double');
            confetti.setAttribute('src', confettiCanvas.toDataURL());
            
            // Animation explosion vers l'extérieur puis chute
            confetti.setAttribute('animation', `property: position; to: ${x * 3} ${-0.5} ${z * 3}; dur: 1500; easing: easeOutQuad`);
            confetti.setAttribute('animation__rotate', `property: rotation; to: ${Math.random() * 360} ${Math.random() * 360} ${Math.random() * 360}; dur: 1500; easing: linear`);
            confetti.setAttribute('animation__fade', `property: components.material.material.opacity; from: 1; to: 0; dur: 1500; delay: 500`);
            
            arContainer.appendChild(confetti);
    ...
    onMarkerLost(question) {
        if (this.currentMarker === question.markerId && !this.answeredQuestions.has(question.id)) {
            ...
            // Supprimer overlay cliquable AR
            const arOverlay = document.getElementById('ar-click-overlay');
            if (arOverlay) arOverlay.remove();
        }
    }

    hideQuestion() {
        // Nettoyer l'overlay si présent
        if (this.clickOverlay && this.clickOverlay.parentNode) {
            document.body.removeChild(this.clickOverlay);
            this.clickOverlay = null;
        }
